import { useState, useEffect, useRef } from 'react'
import { motion, AnimatePresence } from 'framer-motion'
import { IoAdd, IoRemove, IoCheckmarkCircle, IoTicketOutline } from 'react-icons/io5'
import { useCart } from '../context/CartContext'
import { getDiscountedPrice } from '../data/events'
import { formatRupiah } from '../utils/currency'

/* ── Helper Component: Quantity Selector & Add Button ── */
const TicketActions = ({ ticket, qty, isAdded, onUpdateQty, onAddToCart }) => {
  if (ticket.available <= 0) {
    return (
      <span className="text-red-400 font-semibold text-sm px-5 py-2.5 rounded-lg bg-red-500/10">
        Habis Terjual
      </span>
    )
  }

  return (
    <div className="flex items-center gap-3 ml-7 md:ml-0">
      {/* Selector +/- */}
      <div className="flex items-center gap-0 rounded-lg overflow-hidden bg-white/6 border border-white/10">
        <button
          type="button"
          onClick={() => onUpdateQty(ticket.id, -1)}
          className="w-9 h-9 flex items-center justify-center text-white/60 hover:text-white cursor-pointer bg-transparent border-none transition-colors"
        >
          <IoRemove />
        </button>
        <span className="w-10 text-center text-white font-semibold text-sm">{qty}</span>
        <button
          type="button"
          onClick={() => onUpdateQty(ticket.id, 1)}
          className="w-9 h-9 flex items-center justify-center text-white/60 hover:text-white cursor-pointer bg-transparent border-none transition-colors"
        >
          <IoAdd />
        </button>
      </div>

      {/* Add to Cart / Success Message */}
      <AnimatePresence mode="wait">
        {isAdded ? (
          <motion.div
            key="added"
            initial={{ scale: 0.8, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            exit={{ scale: 0.8, opacity: 0 }}
            className="flex items-center gap-1 text-emerald-400 text-sm font-semibold"
          >
            <IoCheckmarkCircle className="text-lg" />
            Ditambahkan!
          </motion.div>
        ) : (
          <motion.button
            key="button"
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            onClick={() => onAddToCart(ticket)}
            disabled={qty === 0}
            className={`px-5 py-2.5 rounded-lg text-sm font-semibold border-none transition-all ${
              qty > 0 
                ? 'text-white cursor-pointer bg-gradient-to-br from-orange-500 to-orange-600 shadow-[0_4px_15px_rgba(249,115,22,0.3)]' 
                : 'text-white/30 cursor-not-allowed bg-white/6'
            }`}
          >
            Tambah ke Keranjang
          </motion.button>
        )}
      </AnimatePresence>
    </div>
  )
}

/* ── Helper Component: Single Ticket Card ────────────── */
const TicketCard = ({ ticket, event, qty, inCart, isAdded, onUpdateQty, onAddToCart }) => {
  const discountedPrice = getDiscountedPrice(ticket.price, event.discount)

  return (
    <motion.div
      whileHover={{ scale: 1.01 }}
      className={`rounded-xl p-5 transition-all bg-white/4 border ${
        qty > 0 ? 'border-orange-500/40' : 'border-white/8'
      }`}
    >
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        {/* Ticket Info */}
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-1">
            <IoTicketOutline className="text-orange-400 text-lg" />
            <h4 className="text-white font-bold text-base m-0">{ticket.type}</h4>
          </div>
          <p className="text-white/50 text-sm mb-2 ml-7">{ticket.description}</p>
          
          {/* Price & Availability */}
          <div className="flex items-center gap-4 ml-7 flex-wrap">
            {event.discount ? (
              <div className="flex items-center gap-2">
                <span className="text-white/40 line-through text-sm">{formatRupiah(ticket.price)}</span>
                <span className="text-white font-bold text-xl">{formatRupiah(discountedPrice)}</span>
                <span className="text-xs font-semibold px-2 py-0.5 rounded-full text-emerald-300 bg-emerald-500/15">
                  -{event.discount.percentage}%
                </span>
              </div>
            ) : (
              <span className="text-white font-bold text-xl">{formatRupiah(ticket.price)}</span>
            )}
            
            <span className="text-white/30 text-xs">
              {ticket.available > 0 ? `${ticket.available} tersisa` : 'Habis Terjual'}
            </span>

            {inCart > 0 && (
              <span className="text-emerald-400 text-xs flex items-center gap-1">
                <IoCheckmarkCircle />
                {inCart} di keranjang
              </span>
            )}
          </div>
        </div>

        {/* Actions */}
        <TicketActions
          ticket={ticket}
          qty={qty}
          isAdded={isAdded}
          onUpdateQty={onUpdateQty}
          onAddToCart={onAddToCart}
        />
      </div>
    </motion.div>
  )
}

/* ── Main Component ──────────────────────────────────── */
function TicketSelection({ event }) {
  const { addTicket, items } = useCart()
  const [quantities, setQuantities] = useState({})
  const [addedTickets, setAddedTickets] = useState({})
  const timeoutRefs = useRef({})

  // Bersihkan timer jika komponen di-unmount agar tidak terjadi memory leak
  useEffect(() => {
    return () => {
      Object.values(timeoutRefs.current).forEach(clearTimeout)
    }
  }, [])

  const getQuantity = (ticketId) => quantities[ticketId] || 0

  const updateQty = (ticketId, delta) => {
    setQuantities(prev => {
      const current = prev[ticketId] || 0
      const nextQty = Math.max(0, Math.min(10, current + delta))
      return { ...prev, [ticketId]: nextQty }
    })
  }

  const handleAddToCart = (ticket) => {
    const qty = getQuantity(ticket.id)
    if (qty === 0) return

    addTicket({
      id: ticket.id,
      eventId: event.id,
      eventTitle: event.title,
      organizerName: event.organizer || 'Unknown Organizer',
      eventDate: event.date,
      eventImage: event.thumbnail,
      name: ticket.type,
      price: getDiscountedPrice(ticket.price, event.discount),
      originalPrice: event.discount ? ticket.price : undefined,
      quantity: qty,
    })

    // Set UI state untuk notifikasi sukses
    setAddedTickets(prev => ({ ...prev, [ticket.id]: true }))
    setQuantities(prev => ({ ...prev, [ticket.id]: 0 }))

    // Bersihkan timeout lama jika tombol diklik cepat berkali-kali
    if (timeoutRefs.current[ticket.id]) {
      clearTimeout(timeoutRefs.current[ticket.id])
    }

    timeoutRefs.current[ticket.id] = setTimeout(() => {
      setAddedTickets(prev => ({ ...prev, [ticket.id]: false }))
    }, 2000)
  }

  const getCartCount = (ticketId) => {
    const found = items.find(item => item.id === ticketId && item.itemType === 'ticket')
    return found ? found.quantity : 0
  }

  return (
    <div className="space-y-4">
      {event.tickets.map((ticket) => (
        <TicketCard
          key={ticket.id}
          ticket={ticket}
          event={event}
          qty={getQuantity(ticket.id)}
          inCart={getCartCount(ticket.id)}
          isAdded={!!addedTickets[ticket.id]}
          onUpdateQty={updateQty}
          onAddToCart={handleAddToCart}
        />
      ))}

      {/* Ticket Info Notice */}
      <div className="rounded-xl p-4 mt-4 bg-indigo-500/10 border border-indigo-500/20">
        <p className="text-indigo-300 text-sm m-0">
          <strong>Catatan:</strong> Tiket tidak dapat dikembalikan. Anda dapat membeli hingga 10 tiket per jenis. 
          E-tiket akan dikirim ke email Anda setelah checkout.
        </p>
      </div>
    </div>
  )
}

export default TicketSelection